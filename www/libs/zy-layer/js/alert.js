...
function Alert(message,action) {
    ...
    this.action = action;
    //首先传入一个函数而不执行他,在内部处理完毕后再回头来调用这个函数  叫做callback
}
//遮罩层
...
Alert.prototype.showButton = function () {
    ...
    btn.onclick = function(){
        self.close();
        //判断self.action是否有值并是否是一个函数
        if(typeof self.action === 'function'){
            self.action();
        }
       /* 事件处理函数中的this指向当前监听事件的标签元素
        所以在这个函数中无法使用this表示Alert对象
        可以使用变量self 把this赋值给self,来保存Alert对象,变量self代替了this
        */
    }
};
//点击关闭
